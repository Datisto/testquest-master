...
query {
    allUserAnswers(condition: {courseId: "11"}, orderBy: GROUP_NAME_ASC) {
        nodes {
            courseId
            groupName
            usrByUserId {
                id
                fullName
                userAnswersByUserId(condition: {courseId: "11"}, orderBy: TASK_NUMBER_ASC) {
                    nodes{
                        taskNumber
...
export default class WSTestResults extends React.Component<{}, {
    pcak: any
}>{
                    ...
                    {({loading, error, data}: { loading: boolean, error?: ApolloError, data: any }) => {
                        ...
                        return (
                            <div className={styles.pageResult}>
                                ...
                                <span className={styles.resultHeader} style={{fontWeight: "normal"}}>ТЕХНИКА БЕЗОПАСНОСТИ НА</span>
                                <span className={styles.resultHeader} style={{fontWeight: "normal"}}>РАБОЧЕМ МЕСТЕ ПРОГРАММИСТА</span>
